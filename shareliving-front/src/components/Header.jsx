...
const Header = ({ isLogged }) => {
  // isLogged를 destructure하여 props에서 가져옵니다.
  ...
  const handleNav = () => {
    setNavStatus(!isMobileNavOn);
  };

  const handleUser = () => {
    setUserControlStatus(!isUserControlsOn);
  };
      ...
      <header id="headPrimary">
        <div className="header-nav-area fullscreen">
          ...
          <nav id="navPrimary">
            <ul className="nav-list">
              ...
              {/* 로그인 상태에 따라 마이 페이지 메뉴 항목을 조건부로 렌더링 */}
              {isLogged && (
                <li className="nav-item">
        ...
        <div className="header-mobile-buttons">
          <button
            type="button"
            id="btnBurger"
            className={isMobileNavOn ? 'on' : null}
            onClick={handleNav}
          >
          ...
          <button
            type="button"
            id="btnUserControl"
            className={isUserControlsOn ? 'on' : null}
            onClick={handleUser}
          >
            <svg
        ...
        <UserControls isMobile={isUserControlsOn} />
      </header>

      <aside id="navToggle" className={isMobileNavOn ? 'on' : null}>
